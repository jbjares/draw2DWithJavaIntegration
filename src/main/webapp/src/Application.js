...
draw2d.Connection.createConnection=function(sourcePort, targetPort, callback, dropTarget){
    ...
    var context = $('<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">'+
                    '<li><a data-connector="example.shape.HasConnection" tabindex="-1" href="#">has</a></li>'+
                    '<li><a data-connector="example.shape.DecreaseConnection" tabindex="-1" href="#">decreases with</a></li>'+
                    '<li><a data-connector="example.shape.IncreaseConnection" tabindex="-1" href="#">increases with</a></li>'+
                    '<li><a data-connector="example.shape.DependsConnection" tabindex="-1" href="#">depends on</a></li>'+
                    '<li><a data-connector="example.shape.ProportionalConnection" tabindex="-1" href="#">is proportional to</a></li>'+
                    '</ul>');
    $("body").append(context);
    ...
    context.show()
           ...
           .find("a").on("click", function(){
                ...
                console.log('ativa ligacao entre conectores');
                callback(con);
           });
};
...
example.Application = Class.extend(
{
    ...
    {    	
        ...
        this.backend = new example.backend.Backend_rest();
