function showCodeDialog(){
	//(new example.dialog.RegexEditorDialog(this.figure,'code')).show();
	(new example.dialog.TemplateEditorDialog(this.figure,'code','teste')).show();
...
example.propertypane.PropertyPaneState = Class.extend({
	...
	injectPropertyView: function( domId)
	{
	    var view = $("<form class='form-horizontal'>"+
	                 ...
	                 "		Code <img class='open_regexpr_editor' src='img/open_in_new_window.png' onclick='showCodeDialog()'>"+
	                 "   </div>"+
	                 "</div>"+
	                 "</form>");
